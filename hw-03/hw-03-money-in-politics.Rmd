...
```{marginfigure}
__Hints:__ You can test your function by defining the URLs for 2020, 2018, and 1998 contributions and using these URLs as inputs. Use this to check the function does what you expected it to do.
```
...
2.  Construct a vector called `urls` that contains the URLs for each webpage that contains information on foreign-connected PAC contributions for a given year.
    ...
    Then write the data frame to a csv file called `pac-all.csv` in the `data` folder.
...
```{marginfigure}
If you can't get the webscraping section above working, a `pac_all.csv` already exists in your repository for you to work on.
```

1.  In your R Markdown file, load `pac-all.csv` and report its number of observations and variables as a full sentence.

```{marginfigure}
Country and parent company names are separated by \\\\, with some entries having two \\\\ (we want to split at the first \\\\).
...
```{marginfigure}
__Hints:__ The `$` character is a special character so it will need to be escaped. Also some contribution amounts are in the millions (e.g. Anheuser-Busch contributed a total of \$1,510,897 in 2008).
...
```{marginfigure}
__Hints:__ These are only UK contributions. You will need to make use of functions from the **scales** package for axis labels as well as from **ggplot2**.
```
